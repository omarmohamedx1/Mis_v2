...
internal static class AdminPermissionCatalog
{
    ...
    public static readonly IReadOnlyCollection<AdminPermissionDefinitionDto> All = new[]
    {
        ...
        P("hr.excuses.view","HR","عرض الأعذار والمأموريات","View excuses","عرض طلبات الأعذار.","View excuse requests.","HIGH",GlobalScopes),
        P("hr.excuses.manage","HR","إدارة الأعذار والمأموريات","Manage excuses","إدارة الأعذار والمرفقات.","Manage excuses and attachments.","HIGH",GlobalScopes),
        P("hr.excuses.approve","HR","اعتماد الأعذار والمأموريات","Approve excuses","اعتماد ورفض الأعذار.","Approve and reject excuses.","HIGH",GlobalScopes),
        P("hr.social_insurance.view","HR","عرض التأمينات الاجتماعية","View social insurance","عرض سجلات التأمين والأجر التأميني لجميع الموظفين.","View insurance records and insurable salary for all employees.","HIGH",GlobalScopes),
        P("hr.social_insurance.manage","HR","إدارة التأمينات الاجتماعية","Manage social insurance","إنشاء وتعديل وإنهاء سجلات التأمين لجميع الموظفين.","Create, edit and end insurance records for all employees.","HIGH",GlobalScopes),
        ...
        P("accounting.payroll.manage","ACCOUNTING","إدارة المرتبات","Manage payroll","إنشاء وتعديل مسيرات الرواتب.","Create and edit payroll runs.","HIGH",DepartmentScopes),
        P("accounting.payroll.approve","ACCOUNTING","اعتماد المرتبات","Approve payroll","اعتماد وصرف المرتبات.","Approve and pay payroll.","CRITICAL",DepartmentScopes),
        P("accounting.transportation.manage","ACCOUNTING","إدارة الانتقالات","Manage transportation","تسجيل واعتماد الانتقالات.","Record and manage transportation claims.","HIGH",DepartmentScopes),
        P("accounting.commission.manage","ACCOUNTING","إدارة العمولات","Manage commissions","حساب وتعديل عمولات التحصيل.","Calculate and adjust collection commissions.","HIGH",DepartmentScopes),
        P("accounting.commission.approve","ACCOUNTING","اعتماد العمولات","Approve commissions","اعتماد وصرف العمولات.","Approve and pay commissions.","CRITICAL",DepartmentScopes),
        ...
        P("legal.access","LEGAL","دخول الشؤون القانونية","Legal access","فتح طابور القضايا المحوّلة من التحصيل بحالة LEGAL.","Open the legal queue of collection cases referred with LEGAL status.","LOW",DepartmentScopes),
        P("legal.case.manage","LEGAL","إدارة القضايا","Manage legal cases","تحديث الملف القانوني وتسجيل الإعلانات والجلسات والأحكام والتسويات والإعادة للتحصيل.","Update the legal file and record notices, hearings, judgments, settlements, and returns to collections.","HIGH",DepartmentScopes),
        ...
        P("data_entry.manage","DATA_ENTRY","إدارة إدخال البيانات","Manage data entry","إنشاء العملاء ورفع دفعات إدخال البيانات.","Create clients and upload data-entry batches.","HIGH",DepartmentScopes),
        ...
        P("collections.data_batch.review","COLLECTIONS","مراجعة دفعات إدخال البيانات","Review data-entry batches","قبول أو رفض دفعات إدخال البيانات وإرسالها للتوزيع.","Accept or reject data-entry batches and send them to distribution.","HIGH",CollectionScopes),
