...
export function BankCustomerDetailsPage() {
  ...
  const Field = ({ label, value }: { label: string; value?: string | number | null }) => (
    <div><p className="text-xs font-semibold uppercase tracking-wide text-slate-400">{label}</p><p className="mt-1 font-semibold text-mis-navy" dir="auto">{value == null || value === '' ? '—' : value}</p></div>
  );

  return (
    <div className="space-y-5">
      <Link className="inline-flex items-center gap-2 text-sm font-semibold text-mis-primary" to={`${base}/${bank.id}/customers`}><ArrowLeft className="h-4 w-4 rtl:rotate-180" />{ct('backToCustomers')}</Link>
      ...
      <Card title={ct('basicInformation')}>
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
          ...
          <Field label={ct('address')} value={data.address} />
            ...
            <tbody className="divide-y divide-mis-border">
              {data.cases.map((item) => (
                <tr key={item.caseId}>
                  ...
                  <td className="px-3 py-2"><Link className="font-bold text-mis-primary" to={`${base}/${bank.id}/portfolio?caseId=${item.caseId}`}>{ct('openPortfolioCase')}</Link></td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <Card title={ct('collectionHistory')}>
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
          {[
            [ct('payments'), `${base}/${bank.id}/dcr`, data.payments.length],
            ['PTP', `${base}/${bank.id}/ptp`, data.promises.length],
            [ct('visits'), `${base}/${bank.id}/visits`, data.visits.length],
            [ct('complaints'), `${base}/${bank.id}/complaints`, data.complaints.length],
            [ct('activity'), `${base}/${bank.id}/activity`, data.activities.length],
