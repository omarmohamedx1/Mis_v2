...
const fields = [
  ...
  ['ReferenceNumber', 'Reference / Form Number', 'رقم الاستمارة / المرجع', /^(reference|reference number|form number|رقم الاستمارة|المرجع)$/i, false],
...
export function HrSocialInsuranceImportPage() {
  ...
  const [history, setHistory] = useState<SocialInsuranceImportHistory[]>([]); const [page, setPage] = useState(1);
  ...
  async function sendFile() {
    ...
    await run(async () => { const data = await socialInsuranceImportService.upload(file); setUpload(data); setMapping(mappingFor(data, 0)); setSheetIndex(0); setStep(1); });
  }
  ...
  return <div className="mx-auto max-w-[1500px] space-y-5"><PageHeader title={text('Import Insurance Sheet', 'استيراد ملف التأمينات')} description={text('Upload, map, review, then confirm. Existing active insurance records are skipped.', 'ارفع الملف وعيّن الأعمدة وراجع المعاينة ثم أكد. يتم تجاوز سجلات التأمين النشطة.')} actions={link} />
    ...
    {step === 0 ? <Card padding="lg"><form className="mx-auto max-w-2xl space-y-5" onSubmit={event => { event.preventDefault(); void sendFile(); }}><FileInput accept=".xlsx,.xls,.csv" label={text('Insurance Excel / CSV file', 'ملف التأمينات Excel / CSV')} hint={text('Maximum 20 MB and 2,000 rows. Uploading does not save insurance records.', 'بحد أقصى 20 ميجابايت و2000 صف. رفع الملف لا يحفظ سجلات التأمين.')} onChange={event => setFile(event.target.files?.[0] ?? null)} required /><Button isLoading={busy} type="submit" leftIcon={<Upload className="h-4 w-4" />}>{text('Upload and Continue', 'رفع ومتابعة')}</Button></form></Card> : null}
    {step === 1 && upload && mapping ? <Card padding="lg"><form className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3" onSubmit={event => { event.preventDefault(); void run(async () => { setPreview(await socialInsuranceImportService.preview(upload.id, mapping)); setPage(1); setStep(2); }); }}>
