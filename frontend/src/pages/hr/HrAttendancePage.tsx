...
import { HrAbsencesPage } from './HrAbsencesPage';
...
export function HrAttendancePage() {
  ...
  const tab = searchParams.get('tab') === 'absences' ? 'absences' : 'records';
  const [data, setData] = useState<PagedAttendanceRecords>(emptyPage); const [departments, setDepartments] = useState<MasterDataLookup[]>([]); const [organizations, setOrganizations] = useState<EmployeeOrganizationAssignment[]>([]);
  const [employeeId, setEmployeeId] = useState(() => searchParams.get('employeeId') ?? ''); const [searchInput, setSearchInput] = useState(() => searchParams.get('employee') ?? ''); const [search, setSearch] = useState(() => searchParams.get('employee') ?? ''); const [departmentId, setDepartmentId] = useState(''); const [organizationId, setOrganizationId] = useState(''); const [dateFrom, setDateFrom] = useState(() => searchParams.get('dateFrom') ?? ''); const [dateTo, setDateTo] = useState(() => searchParams.get('dateTo') ?? ''); const [status, setStatus] = useState<AttendanceStatus | ''>(() => { const value = searchParams.get('status'); return attendanceStatuses.includes(value as AttendanceStatus) ? value as AttendanceStatus : ''; }); const [source, setSource] = useState<AttendanceSource | ''>(''); const [page, setPage] = useState(1);
  ...
  useEffect(() => {
    void Promise.all([hrMasterDataService.getLookup('departments'), hrEmployeeService.getOrganizations()])
      .then(([departmentItems, organizationItems]) => { setDepartments(departmentItems); setOrganizations(organizationItems); })
      .catch((reason) => toast.error(getApiErrorMessage(reason, t('loadDepartmentsError'))));
  }, [t, toast]);
  const load = useCallback(async () => {
    setLoading(true); setError('');
    try { setData(await hrAttendanceService.getPaged({ dateFrom, dateTo, departmentId, organizationId, employeeId, page, pageSize, search, source, status, sortBy: 'attendanceDate', sortDescending: true })); }
    catch (requestError) { setError(getApiErrorMessage(requestError, t('loadAttendanceError'))); }
    finally { setLoading(false); }
  }, [dateFrom, dateTo, departmentId, organizationId, employeeId, page, search, source, status, t]);
  useEffect(() => { if (tab === 'records') void load(); }, [load, tab]);
  function changeTab(id: string) {
    const next = new URLSearchParams(searchParams);
    if (id === 'absences') next.set('tab', 'absences');
  ...
  function clearFilters() { setEmployeeId(''); setSearchInput(''); setSearch(''); setDepartmentId(''); setOrganizationId(''); setDateFrom(''); setDateTo(''); setStatus(''); setSource(''); setPage(1); }
  const hasFilters = useMemo(() => Boolean(employeeId || search || departmentId || organizationId || dateFrom || dateTo || status || source), [dateFrom, dateTo, departmentId, organizationId, employeeId, search, source, status]);

  return <div>
    <PageHeader actions={tab === 'records' ? <><Button fullWidth={false} leftIcon={<WandSparkles className="h-4 w-4" />} onClick={() => setProcessOpen(true)} size="md" variant="outline">{t('processDay')}</Button><Link className="inline-flex h-10 items-center justify-center gap-2 rounded-xl border border-mis-border bg-white px-4 text-sm font-semibold text-slate-700 hover:border-mis-blue hover:bg-mis-pale/50 hover:text-mis-primary" to="/hr/attendance/import"><FileUp className="h-4 w-4" />{t('importAttendance')}</Link><Button fullWidth={false} leftIcon={<Plus className="h-4 w-4" />} onClick={() => setFormRecord(null)} size="md">{t('addManualAttendance')}</Button></> : undefined} description={t('attendanceSubtitle')} eyebrow={t('hrDepartment')} title={t('attendance')} />
    <div className="mb-5 overflow-hidden rounded-2xl border border-mis-border bg-white shadow-sm">
      <Tabs ariaLabel={t('attendanceAndAbsencesTabs')} items={[{ id: 'records', label: t('attendanceRecordsTab') }, { id: 'absences', label: t('attendanceAbsencesTab') }]} onChange={changeTab} value={tab} />
    </div>
    {tab === 'absences' ? <HrAbsencesPage embedded /> : <>
    ...
    <section className="overflow-hidden rounded-2xl border border-mis-border bg-white shadow-sm">
      <div className="module-filter-grid border-b border-mis-border p-4">
        ...
        <ProfessionalSelect aria-label={t('department')} className="h-11 rounded-xl border border-mis-border bg-white px-3 text-sm" onChange={(event) => { setDepartmentId(event.target.value); setPage(1); }} value={departmentId}><option value="">{t('allDepartments')}</option>{departments.map((item) => <option key={item.id} value={item.id}>{lookupLabel(item, language)}</option>)}</ProfessionalSelect>
        <ProfessionalSelect aria-label={t('assignedBankCompany')} className="h-11 rounded-xl border border-mis-border bg-white px-3 text-sm" onChange={(event) => { setOrganizationId(event.target.value); setPage(1); }} value={organizationId}><option value="">{t('allOrganizations')}</option>{organizations.map((item) => <option key={item.id} value={item.id}>{organizationLabel(item, language)}</option>)}</ProfessionalSelect>
