...
export function HrSocialInsurancePage({ employeeId: profileEmployeeId }: { employeeId?: string }) {
  const { language, isRtl } = useLocalization(); const w = words[language];
  ...
  useEffect(() => { void hrEmployeeService.getDepartments().then(setDepartments).catch(() => setDepartments([])); }, [language]);
  ...
  return <div className="space-y-5" dir={isRtl ? 'rtl' : 'ltr'}>
    <PageHeader title={w.title} actions={profileEmployeeId ? <Link className="text-mis-primary underline" to={`/hr/social-insurance?employeeId=${profileEmployeeId}`}>{w.full}</Link> : data?.canManage ? <div className="flex flex-wrap items-center gap-2.5 sm:flex-nowrap"><Button className="shrink-0 whitespace-nowrap" fullWidth={false} size="md" leftIcon={<Plus aria-hidden="true" className="h-4 w-4 shrink-0" />} onClick={() => void open('create', null)}>{w.add}</Button><Link className="inline-flex h-10 shrink-0 items-center justify-center gap-2 whitespace-nowrap rounded-xl border border-mis-primary bg-white px-4 text-sm font-bold text-mis-primary shadow-sm transition duration-150 hover:bg-mis-pale" to="/hr/social-insurance/import"><Upload aria-hidden="true" className="h-4 w-4 shrink-0" />{language === 'ar' ? 'استيراد ملف التأمينات' : 'Import Insurance Sheet'}</Link></div> : undefined} />
    {!profileEmployeeId && data && <div className="grid gap-3 sm:grid-cols-3">{[[w.insured, data.totalInsured], [w.notInsured, data.notInsured], [w.ended, data.endedRecords]].map(([label, value]) => <Card key={label} className="p-4"><p className="text-sm text-slate-500">{label}</p><p className="mt-1 text-2xl font-semibold">{value}</p></Card>)}</div>}
    {!employeeId && <div className="grid gap-3 md:grid-cols-[2fr_1fr_1fr]"><TextInput name="insurance-search" label={w.search} value={search} onChange={e => { setSearch(e.target.value); setPage(1); }} /><SelectInput label={w.department} value={departmentId} onChange={e => { setDepartment(e.target.value); setPage(1); }}><option value="">{w.all}</option>{departments.map(d => <option key={d.id} value={d.id}>{d.name}</option>)}</SelectInput><SelectInput label={w.status} value={status} onChange={e => { setStatus(e.target.value); setPage(1); }}><option value="">{w.all}</option>{statuses.map(s => <option key={s} value={s}>{w[s]}</option>)}</SelectInput></div>}
    {message && <p role="status" className="text-emerald-700">{w[message]}</p>}
    {error && <p role="alert" className="text-red-700">{error}</p>}
    {loading ? <LoadingSpinner /> : !error && data && <Card className="overflow-hidden"><div className="overflow-x-auto"><table className="w-full text-start text-sm"><thead className="bg-slate-50"><tr>{headers.map(h => <th key={h} className="whitespace-nowrap px-4 py-3 text-start">{h}</th>)}</tr></thead><tbody>{data.items.map(e => <tr className="border-t border-slate-100" key={e.employeeId}>
      ...
      <td className="min-w-52 px-4 py-3"><div className="flex flex-wrap gap-2 [&>button]:whitespace-nowrap">{e.record && <Button variant="secondary" onClick={() => void open('view', e)}>{w.view}</Button>}{data.canManage && (e.record && e.record.insuranceStatus !== 'Ended' ? <><Button variant="secondary" onClick={() => void open('edit', e)}>{w.edit}</Button><Button variant="secondary" onClick={() => void open('end', e)}>{w.endAction}</Button></> : <Button onClick={() => void open('create', e)}>{w.add}</Button>)}</div></td>
    ...
    <Modal open={dialog !== null} onClose={() => { if (!busy) setDialog(null); }} title={dialog === 'create' ? w.add : dialog === 'edit' ? w.edit : dialog === 'end' ? w.endAction : w.view} size="lg">
      {formError && <p role="alert" className="mb-4 text-red-700">{formError}</p>}
      {dialog === 'view' ? <div className="space-y-6">{selected && <p className="font-semibold">{selected.employeeNumber} · {selected.employeeName}</p>}{record && detail(record)}<h3 className="font-semibold">{w.history}</h3>{busy ? <LoadingSpinner /> : history.map(item => <button type="button" key={item.id} onClick={() => setRecord(item)} className={`block w-full rounded-lg border p-3 text-start ${record?.id === item.id ? 'border-mis-primary bg-slate-50' : 'border-slate-200'}`}>{item.socialInsuranceNumber} · {date(item.insuranceStartDate)} · {w[item.insuranceStatus]}</button>)}</div> : <form onSubmit={submit} className="space-y-5">
        ...
        <div className="flex justify-end gap-3"><Button type="button" variant="secondary" disabled={busy} onClick={() => setDialog(null)}>{w.cancel}</Button><Button type="submit" disabled={busy || dialog !== 'end' && !selected}>{dialog === 'end' ? w.endAction : w.save}</Button></div>
      </form>}
    </Modal>
  </div>;
}
