...
function DelegationForm({ item, entities, onClose, onSaved }: { item: DelegationDetails | null; entities: DelegationEntityOption[]; onClose: () => void; onSaved: (value: DelegationDetails) => void }) {
  ...
  async function selectEmployee(id: string, selected: EmployeeListItem | null) {
    ...
    if (id) try { const profile = await hrEmployeeProfileService.getProfile(id); setNationalId(profile.personal.nationalId ?? ''); } catch { setNationalId(''); setError(tx('تعذر تحميل الرقم القومي للموظف. حدّث الصفحة ثم أعد المحاولة.', 'Unable to load the employee National ID. Refresh and try again.')); }
  }
...
export function HrDelegationsPage() {
  ...
  const [query, setQuery] = useState<DelegationQuery>(() => ({ ...initialQuery, employeeId: searchParams.get('employeeId') ?? '', search: searchParams.get('employee') ?? '' })); const [data, setData] = useState(emptyPage); const [entities, setEntities] = useState<DelegationEntityOption[]>([]);
  const [loading, setLoading] = useState(true); const [error, setError] = useState(''); const [lookupsError, setLookupsError] = useState(''); const [formOpen, setFormOpen] = useState(false); const [editing, setEditing] = useState<DelegationDetails | null>(null); const [preview, setPreview] = useState<DelegationDetails | null>(null); const [cancelTarget, setCancelTarget] = useState<DelegationListItem | null>(null); const [cancelReason, setCancelReason] = useState('');
  ...
  useEffect(() => { const timer = window.setTimeout(() => setQuery(current => ({ ...current, search: searchInput.trim(), page: 1, employeeId: searchInput.trim() !== (searchParams.get('employee') ?? '') ? '' : current.employeeId })), 350); return () => window.clearTimeout(timer); }, [searchInput, searchParams]);
  useEffect(() => { void hrDelegationService.getEntities().then(setEntities).catch((reason) => { setEntities([]); setLookupsError(getApiErrorMessage(reason, tx('تعذر تحميل البنوك والجهات.', 'Unable to load banks and entities.'))); }); }, [language]);
  ...
  const hasFilters = Boolean(query.search || query.employeeId || query.delegatingEntityId || query.status || query.dateFrom || query.dateTo);
  function clearFilters() { setSearchInput(''); setQuery({ ...initialQuery }); }
