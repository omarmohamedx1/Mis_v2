import { Activity, BadgeDollarSign, BarChart3, BriefcaseBusiness, Building2, ClipboardList, FileUp, Gauge, Landmark, MapPinned, Settings, WalletCards } from 'lucide-react';
...
const links = [
  ...
  { to: '/installment-companies', label: 'installmentCompanies', icon: BadgeDollarSign }, { to: '/collections/clients', label: 'clients', icon: Building2 },
  ...
  { to: '/collections/data-batches', labelKey: 'dataBatches', icon: ClipboardList, supervisorOnly: true },
...
export function CollectionsLayout() {
  ...
  const navigation: ModuleNavigationItem[] = links
    ...
    .map((link) => ({
      to: link.to,
      icon: link.icon,
      label: 'labelKey' in link
        ? (ar ? 'دفعات إدخال البيانات' : 'Data batches')
        : ct(link.label),
    }));
