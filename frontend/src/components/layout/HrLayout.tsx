...
const links = [
  ...
  ['/hr/excuses', 'excusesMissions', ScrollText], ['/hr/leaves', 'leaves', CalendarDays], ['/hr/delegations', 'delegations', ScrollText], ['/hr/absences', 'companyAbsences', UserRoundX],
  ['/hr/employee-documents', 'employeeDocuments', FileText], ['/hr/master', 'master', Database], ['/hr/calendar', 'workingCalendar', CalendarDays],
  ['/hr/social-insurance', 'socialInsurance', FileText], ['/hr/reports', 'reports', BarChart3], ['/hr/audit', 'auditHistory', Activity],
] as const;
...
export function HrLayout() {
  ...
  const navigation: ModuleNavigationItem[] = links.filter(([to]) => to !== '/hr/excuses' || user?.roles.some(r => ['HrManager', 'HrOfficer'].includes(r)) || user?.permissions.some(p => ['hr.excuses.view', 'hr.excuses.manage', 'hr.excuses.approve'].includes(p))).filter(([to]) => to !== '/hr/social-insurance' || user?.roles.some(r => ['HrManager', 'HrOfficer'].includes(r)) || user?.permissions.some(p => ['hr.social_insurance.view', 'hr.social_insurance.manage'].includes(p))).map(([to, key, icon]) => ({ to, icon, label: t(key as TranslationKey) }));
  return <ModuleLayoutShell headerAside={<HrExcuseNotifications />} collapseLabel={language === 'ar' ? 'طي القائمة' : 'Collapse navigation'} companyLabel={t('collectionFirm')} expandLabel={language === 'ar' ? 'توسيع القائمة' : 'Expand navigation'} headerTitle={t('humanResources')} isRtl={isRtl} languageLabel={language === 'ar' ? 'English' : 'العربية'} moduleName={t('humanResources')} moduleSubtitle={t('hrDepartment')} navigation={navigation} navigationLabel={t('hrNavigation')} onLanguageToggle={() => setLanguage(language === 'ar' ? 'en' : 'ar')} onSignOut={() => { logout(); navigate('/login', { replace: true }); }} openNavigationLabel={t('openNavigation')} closeNavigationLabel={t('closeNavigation')} profileLabel={language === 'ar' ? 'ملفي الشخصي' : 'My profile'} profilePath="/hr/profile" signOutLabel={t('logout')} storageKey="mis.hr.sidebar" theme="hr" userName={user?.fullName} />;
}
