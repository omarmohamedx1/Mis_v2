...
export interface BankPortfolioImportPreviewRow { rowNumber: number; customerName?: string | null; customerCode?: string | null; accountReference?: string | null; outstandingBalance?: number | null; status: string; errors: string[]; nationalId?: string | null; cardNumber?: string | null; bucket?: string | null; importStatus?: string | null; totalDues?: number | null }
...
export interface BankCustomerDetails {
  ...
  mobile?: string | null; alternativeMobile?: string | null; nationalId?: string | null; address?: string | null; secondaryAddress?: string | null;
...
export interface CollectionDashboardClientSlice { id: string; name: string; organizationType: string; cases: number; assigned: number; unassigned: number; legalCases: number; outstanding: number; overdue: number; code?: string; logoUrl?: string }
export interface CollectionDashboardBucketSlice { code: string; name: string; cases: number; outstanding: number; overdue: number }
export interface CollectionDashboardTrendPoint { date: string; collected: number }
export interface CollectionDashboardStatusSlice { status: string; cases: number; outstanding: number }
export interface CollectionDashboard { totalCases: number; totalOutstanding: number; totalOverdue: number; assignedCases: number; unassignedCases: number; activeCollectors: number; collectedToday: number; collectedMonthToDate: number; achievementPercent: number; activePromises: number; promisesDueToday: number; brokenPromises: number; visitsToday: number; pendingReviews: number; openComplaints: number; highRiskCases: number; overdueFollowUps: number; byClient: CollectionDashboardClientSlice[]; byBucket: CollectionDashboardBucketSlice[]; collectionTrend: CollectionDashboardTrendPoint[]; byStatus: CollectionDashboardStatusSlice[] }
export interface CollectionCreditorDesk { portfolioId: string; name: string; primaryClassification?: string | null; subClassification?: string | null; cases: number; assigned: number; unassigned: number; legalCases: number; outstanding: number; overdue: number }
export interface CollectionCreditorBook { id: string; code: string; name: string; logoUrl?: string; organizationType: string; cases: number; assigned: number; unassigned: number; legalCases: number; outstanding: number; overdue: number; desks: CollectionCreditorDesk[] }
export interface CollectionCreditorDashboard { banks: CollectionCreditorBook[]; companies: CollectionCreditorBook[] }
export interface CollectionCollectorOrgSlice { organizationId: string; code?: string; name: string; logoUrl?: string; organizationType: string; cases: number; outstanding: number; overdue: number; legalCases: number }
export interface CollectionCollectorBook { id: string; name: string; loginCode: string; cases: number; outstanding: number; overdue: number; legalCases: number; highRiskCases: number; organizations: CollectionCollectorOrgSlice[] }
...
export interface CaseDetails { id: string; caseNumber: string; clientName: string; portfolioName: string; customerCode: string; customerName: string; nationalId: string; primaryPhone: string; alternatePhone: string; tertiaryPhone?: string; address: string; secondaryAddress?: string; governorate?: string; area?: string; city?: string; employer?: string; jobTitle?: string; feedback?: string; accountReference: string; cardNumber?: string; contractReference?: string; productType?: string; importStatusText?: string; stage?: string; originalAmount: number; outstandingBalance: number; overdueBalance: number; penalties: number; fees: number; totalDue: number; daysPastDue: number; bucket: string; importBucketLabel?: string; status: string; priority: string; priorityScore: number; priorityExplanation: string; assignedCollectorName?: string; assignedCollectorId?: string; previousCollectorName?: string; fileCollectorName?: string; previousCollectorUserId?: string; previousCollectorUserName?: string; fileCollectorUserId?: string; fileCollectorUserName?: string; creditLimit?: number; purchaseAvailableLimit?: number; activationDate?: string; lastPaymentAmount?: number; lastPaymentAt?: string; lastTransactionDate?: string; lastTransactionAmount?: number; sensitiveValuesRevealed: boolean; recommendedActionCode: string; recommendedActionReason: string; fileSnapshot?: CollectionFileSnapshot | null; relatedCreditorCases: RelatedCreditorCase[]; timeline: Activity[]; promises: PromiseItem[]; payments: PaymentItem[] }
...
export interface ImportedCollectorLinkSlice { collectorId: string; collectorName: string; fileName: string; cases: number }
export interface ImportedCollectorLinkResult { linkedCases: number; ambiguousCases: number; unmatchedCases: number; byCollector: ImportedCollectorLinkSlice[] }
...
export interface PortfolioConfiguration { id: string; organizationId: string; code: string; nameArabic: string; nameEnglish: string; currencyCode: string; targetAmount?: number; primaryClassification?: string; subClassification?: string; ptpGraceDays?: number; ptpToleranceAmount?: number; isActive: boolean; caseCount?: number }
export interface SavePortfolioConfiguration { organizationId: string; code: string; nameArabic: string; nameEnglish: string; currencyCode: string; targetAmount?: number; primaryClassification?: string; subClassification?: string; ptpGraceDays?: number; ptpToleranceAmount?: number; isActive: boolean }
...
export interface SaveBucketConfiguration { organizationId: string; portfolioId?: string; code: string; nameArabic: string; nameEnglish: string; minimumDays?: number; maximumDays?: number; sortOrder: number; isActive: boolean }
...
export interface CaseFilters { page?: number; pageSize?: number; search?: string; searchField?: string; organizationId?: string; portfolioId?: string; collectorId?: string; bucket?: string; status?: string; priority?: string; unassigned?: boolean }
