...
export interface SocialInsuranceImportMapping { sheetName: string | null; headerRow: number; firstDataRow: number; dateFormat: string | null; columns: Record<string, string>; sheetNames?: string[] }
...
export const socialInsuranceImportService = {
  ...
  async confirm(id: string, previewId: string, excludedRows?: number[]) { return (await apiClient.post<SocialInsuranceImportResult>(`${base}/${id}/confirm`, { previewId, excludedRows })).data; },
