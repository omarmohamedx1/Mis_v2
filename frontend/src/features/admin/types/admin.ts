...
export interface AdminUser { id: string; loginCode: string; username: string; email: string; fullName: string; departmentId: string; departmentCode: string; departmentNameAr: string; departmentNameEn: string; isActive: boolean; createdAt: string; lastLoginAt?: string; mustChangePassword: boolean; roles: AdminRole[]; accessGrants: AdminAccessGrant[]; employeeId?: string; employeeNumber?: string; employeeName?: string; employeeOperationalRole?: string }
export interface AdminLinkableEmployee { id: string; employeeNumber: string; fullName: string; departmentId: string; departmentCode: string; departmentNameAr: string; departmentNameEn: string; operationalRole?: string; email?: string; linkedUserId?: string }
export interface AdminCredentialIssue { user: AdminUser; temporaryPassword: string }
...
export interface AdminDashboard { totalUsers: number; activeUsers: number; inactiveUsers: number; pendingAccessRequests: number; privilegedUsers: number; expiringAccessCount: number; neverLoggedInCount: number; mustChangePasswordCount: number; decisionQueue: AdminDecisionItem[]; departments: AdminDepartmentSummary[]; recentActivity: AdminAuditItem[] }
...
export interface CreateAdminUser { fullName: string; username: string; email: string; departmentId: string; temporaryPassword?: string; roleIds: string[]; employeeId?: string }
