...
export const financeService = {
  ...
  async journals(status?: string) { return loadAllPages((page, pageSize) => apiClient.get<FinancePagedResult<JournalListItem>>('/finance/journals', { params: { page, pageSize, status: status || undefined } }).then(r => r.data), 200); },
  ...
  async collections(status?: string, channel?: string) { return loadAllPages((page, pageSize) => apiClient.get<FinancePagedResult<CollectionFinanceListItem>>('/finance/collections', { params: { page, pageSize, status: status || undefined, channel: channel || undefined } }).then(r => r.data), 200); },
